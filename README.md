...
For large pools, `management.probe_mode: adaptive` supports both `probe_max_per_hour` (default 600) and `probe_max_per_day` (default 5000). The stricter remaining budget wins, recent successful real traffic suppresses redundant active probes, and exhausted work is deferred to the next reset window.
